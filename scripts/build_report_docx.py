...
def build():
    ...
    B(
        ...
        "탈추세 값을 0합으로 재중심화한다. 실제 CPU는 시간 간 강하게 자기상관되므로, "
        "결정론적 분해에 AR(1) 잔차 보정(ŷ += ρ^h·최근잔차, ρ는 잔차의 시차-1 자기상관)을 "
        "더해 최근 수준을 반영한다. 이 항이 모델을 두 기준선보다 우수하게 만든다(§7). "
        "예측 구간은 표본 외 1-스텝 백테스트 RMSE로 산정하며, 엔진은 표준 라이브러리만 "
        "사용해 네이티브 의존성·GPU가 필요 없다."
    )

    # --------------------------------------------------- 4. 정수계획 리사이징
    H1("4. 정수계획 리사이징 및 실제 적용")
    B(
        "현재 할당과 예측 피크 부하가 주어지면, 엔진은 안전 마진을 유지하면서 예측 피크 "
        "가동률을 목표 이하로 유지하는 최소 정수 할당을 선택한다: peak_load × margin ≤ "
        "target_utilisation × allocation. 탐색 공간이 작고 유계(vCPU∈[1,현재], 메모리 "
        "256MB 블록)이므로 전수 열거로 정확 해를 구한다. 95퍼센타일 피크 통계로 단발 "
        "스파이크의 영구 과프로비저닝을 방지한다(버스트 호스트는 자동 유지됨)."
    )
    H2("4.1. GCP 머신 타입 매핑과 예산 가드 실제 리사이즈")
    B(
